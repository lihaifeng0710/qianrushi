...
IRQn_Type table_irq_uart_dma[3] = {UART0_IRQn, UART1_IRQn, UART2_IRQn};
...
{
	uint8_t UART,chSend,chRe; //串口模块和通道
	//解析UART_DMA
	if(UART_DMA==0)
	{
		UART=0;//串口0
...
//    }
    NVIC_EnableIRQ(table_irq_uart_dma[uartNo]);    //开中断控制器IRQ中断
}
...
//函数返回：无
//======================================================================
 void dma_enable(uint8_t UART_DMA)
{
	uint8_t UART,chSend,chRe; //串口模块和通道
	//解析UART_DMA
	if(UART_DMA==0)
