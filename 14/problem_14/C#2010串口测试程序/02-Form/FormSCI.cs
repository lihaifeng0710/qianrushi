    ...
    public partial class FrmSCI : Form
    {
        ...
        private void BtnSCISend_Click(object sender, EventArgs e)
        {
            ...
            if (SendType == 0)//选择的是以字符串方式发送
            {
                ...
                string len_1 = len.ToString();
                SendData.Add(Convert.ToByte(len_1, 10));
                PublicVar.g_SendByteArray = new byte[1];
                PublicVar.g_SendByteArray[0] = (byte)SendData[0];
                Flag1 = sci.SCISendData(ref PublicVar.g_SendByteArray);//发送字符数
                ...
                Flag = sci.SCISendData(ref PublicVar.g_SendByteArray);//发送字符
                if (Flag == true && Flag1 == true)//数据发送成功
                    this.TSSLState.Text += SendData[0];
                else
        ...
        {
            string time_string;
            time_string = DateTime.Now.ToLongTimeString().ToString();
            this.TSSLState.Text = "过程提示: 执行发送数据...";
            ...
            if (SendType == 0)//选择的是以字符串方式发送
            {
                this.TSSLState.Text = "以字符串方式发送数据!";
                //将要发送的数据进行编码,并获取编码后的数据长度
                len = System.Text.Encoding.Default.GetBytes(time_string).Length;
                //sci.SCIReceInt(SCIPort,len);//设置产生接收中断的字节数  【2014-5-5 注释，否则会导致程序无响应】
                //动态分配len字节单元内容用来存放发送数据
                PublicVar.g_SendByteArray = new byte[len];
                //获取TbSCISend文本的码值
                PublicVar.g_SendByteArray =System.Text.Encoding.Default.GetBytes(time_string);
            } 
            else //选择的是以十进制或者是十六进制发送数据
            {
                //sci.SCIReceInt(SCIPort, 1);//设置产生接收中断的字节数    【2014-5-5 注释，否则会导致程序无响应】
                foreach (string str in time_string.Split(':'))
                {
                    //排除掉连续两个逗号,说明之间没有数
                    if (str != string.Empty)
                    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            _text_component = File.ReadAllText(@"C: \Users\user\Desktop\test.txt");
            this.TSSLState.Text = "过程提示: 执行发送数据...";
