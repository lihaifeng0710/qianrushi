...
static const IRQn_Type I2C_IRQ_TABLE[2] = {I2C0_IRQn,I2C1_IRQn};
 ...
 //函数返回：无
 //=====================================================================
void i2c_init(uint_8 No,uint_8 Mode,uint_8 address,uint_8 BaudRate)
{
	ModeSelect = Mode;
...
//=====================================================================
uint_8 i2c_read1(uint_8 No,uint_8 slaveAddress,uint_16 command,uint_8 *Data)
{      
	uint_8 temp;
	i2c_start(No);               //发送开始信号
	if(command!=0xFFFF)
...
//函数返回： 1：表示读取失败；0：读取成功
//=====================================================================
uint_8 i2c_readN(uint_8 No,uint_8 slaveAddress,uint_16 command,uint_8 *Data, uint_8 DataNum)
{
	uint_8 i,temp;
	i2c_start(No);               //发送开始信号
	if(command!=0xFFFF)
...
//=====================================================================
uint_8 i2c_write1(uint_8 No, uint_8 slaveAddress, uint_8 Data,uint_16 command)
{
	i2c_start(No);               //发送开始信号
...
//函数说明: 1：表示发送失败；0：发送成功
//=====================================================================
uint_8 i2c_writeN(uint_8 No, uint_8 slaveAddress, uint_8 *Data, uint_8 DataNum)
{
	uint_8 i;
	i2c_start(No);               //发送开始信号
	slaveAddress = (slaveAddress << 1) & 0xfe;
...
//函数返回：无。
//==============================================================================
void i2c_enable_re_int(uint_8 No)
{
	I2C_MemMapPtr i2c_ptr = I2C_BASE_TABLE[No];
	i2c_ptr->S |= I2C_S_IICIF_MASK;
	i2c_ptr->C1 |= I2C_C1_IICIE_MASK;
	enable_irq (I2C_IRQ_TABLE[No]);
}
...
//函数返回：无。
//==============================================================================
void i2c_disable_re_int(uint_8 No)
{
	I2C_MemMapPtr i2c_ptr = I2C_BASE_TABLE[No];
	i2c_ptr->C1 &= ~I2C_C1_IICIE_MASK;
...
//函数返回：无。
//=====================================================================
void i2c_clear_re_int(uint_8 No)
{
	I2C_MemMapPtr i2c_ptr = I2C_BASE_TABLE[No];
	i2c_ptr->S|=0x02;
}
...
//函数返回： 无
//=====================================================================
void i2c_write(uint_8 No,uint_8 data)
{
	I2C_MemMapPtr i2c_ptr = I2C_BASE_TABLE[No];
	i2c_ptr->D = data;
}
...
//函数返回： 无
//=====================================================================
void i2c_changeDir(uint_8 No,uint_8 direction)
{
	I2C_MemMapPtr i2c_ptr = I2C_BASE_TABLE[No];
